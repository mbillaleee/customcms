            ...
            @csrf
                <div class="card p-5">
                    <div class="mt-5">
                        ...
                        <input type="text" id="input" class="form-control" name="title" value="{{ $blog->title }}" placeholder="Enter text here">
                    </div>
                    <div class="mt-5">
                        <label class="label block mb-2">Image</label>
                        <input type="file" id="input" class="form-control" name="image">
                        @if($blog->image != null)<img src="{{asset('uploads/blog/'.$blog->image)}}" width="80" alt="{{$blog->title}}">@endif
                    </div>
                    <div class="mt-5">
                    ...
                    <div class="mt-5">
                        ...
                        <textarea type="text" class="form-control" rows="5" name="content" value="">{{ $blog->content }}</textarea>
                    </div>
                    <div class="mt-5">
                        <label class="label block mb-2">Status</label>
                        <input type="radio" name="status" value="1" {{ $blog ? $blog->status == 1 ? "checked" : "": ""   }}><span>Active</span>
                        <input type="radio" name="status" value="0" {{ $blog ? $blog->status == 0 ? "checked" : "": ""   }}><span>Inactive</span>
                    </div>
                    <div class="mt-5">
