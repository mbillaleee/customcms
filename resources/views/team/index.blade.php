...
@section('workspace') 
    ...
    <div class="grid grid-rows-1 grid-flow-col gap-4">
        ...
        <div class="flex flex-col gap-y-5">
            <div class="card p-5">
            <a class="btn btn_primary float-end" href="{{ route('team.create') }}"> Add Team</a>
                <table class="table table_striped w-full mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->name }}</td>
                            <td>{{ $item->email }}</td>
                            <td>@if($item->image != null)<img src="{{asset('uploads/team/'.$item->image)}}" width="80" alt="$item->name">@endif</td>
                            <td>{!! !!$item->status ? '<span class="badge bg-success">Active</span>': '<span
                                                class="badge bg-danger">Inactive</span>' !!}</td>
                            <td class="flex">
                            <a class="bg-blue-500 text-white hover:text-white font-bold py-1 px-4 rounded-full" href="{{ route('team.edit',$item->id) }}">Edit</a>
                                <form class="" method="POST" action="{{route('team.destroy', $item->id)}}">
